...
class TimelensApp:
    ...
    def __init__(self) -> None:
        ...
        # Thread safe objects
        self._local_redis_factory = RedisClientFactory.from_local_defaults()
        self._remote_redis_factory = RedisClientFactory.from_remote_defaults()
        self._local_redis_client = self._local_redis_factory.new_redis_client()
        self._remote_redis_client = self._remote_redis_factory.new_redis_client()
        ...
        self.remote_job_manager_cpu_bound = JobManager(
            self._remote_redis_client, JobQueue.REMOTE_MAIN_TASK_QUEUE_CPU_BOUND
        )
